        		...
        		<form method="post" id="user_form">
        			...
        			<div class="modal-body">
                        ...
                        <div class="form-group" style="display: flex; flex-direction: column; align-items: start;">
                            <label>If Unique User Name</label>
                            <small class="text-info">If this checkbox is checked, the system will verify that the username is unique.</small>
                            <input type="checkbox" name="user_name_unique" value="true" style="height: 2.5rem; width: 2.5rem; border-radius: 10px;"/>
                        </div>
        			</div>
	...
	$(document).on('submit', '#user_form', function(event){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('#alert_action').fadeIn().html('<div class="alert <?php if(isset($_SESSION['error_type']) && $_SESSION['error_type'] == 'error') echo "alert-danger"?> "alert-success">'+
                    data
                    +'</div>');
