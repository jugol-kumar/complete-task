...
//	require_once 'pdf.php';
    ...
    $statement = mysqli_query($connect, "
		SELECT * FROM inventory_order 
		WHERE inventory_order_id = '".$_GET["order_id"]."' LIMIT 1 ");
...
//    foreach($statement->fetch_assoc() as $row)
//    {
...
//		);
        ...
        $statement = mysqli_query($connect, "
			SELECT * FROM inventory_order_product 
			WHERE inventory_order_id = '".$_GET['order_id']."'
		");
		...
		while($sub_row = $statement->fetch_assoc())
		{

//            $sub_row = $statement->fetch_assoc();
